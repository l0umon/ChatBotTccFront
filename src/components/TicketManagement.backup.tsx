...
const TicketManagement: React.FC = () => {
  ...
  const loadTickets = useCallback(async (isInitial = false) => {
    try {
      ...
      setTickets([]); // Asegurar que se limpia en caso de error
    } finally {
      if (isInitial) {
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
            <button
              ...
              onMouseEnter={(e) => {
                if (!loading) e.currentTarget.style.background = 'rgba(102, 126, 234, 0.2)';
              }}
              onMouseLeave={(e) => {
                if (!loading) e.currentTarget.style.background = 'rgba(102, 126, 234, 0.1)';
              }}
            >
            ...
            <div style={{ flex: 1, overflow: 'auto', padding: '24px' }}>
              ...
              <div style={{ marginBottom: '24px' }}>
                ...
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: '12px', marginBottom: '16px' }}>
                  <div style={{
                    background: getStatusColor(selectedTicket.estado),
                    color: 'white',
                    padding: '6px 12px',
                    borderRadius: '8px',
                    fontSize: '12px',
                  ...
                  <div style={{
                    background: getPriorityColor(selectedTicket.prioridad),
                    color: 'white',
                    padding: '6px 12px',
                    borderRadius: '8px',
                    fontSize: '12px',
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '16px', fontSize: '14px', color: '#64748b' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                    <User size={16} />
                    <span>{selectedTicket.usuario.nombre} {selectedTicket.usuario.apellido}</span>
                  </div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                    <Calendar size={16} />
              ...
              {/* Contexto del Chat */}
              {selectedTicket.contextoChat && selectedTicket.contextoChat.length > 0 && (
                <div style={{ marginBottom: '24px' }}>
                  ...
                  <div style={{
                    ...
                    {selectedTicket.contextoChat.map((msg, index) => (
                      <div key={index} style={{ marginBottom: '12px', display: 'flex', gap: '8px' }}>
                        <div style={{
                          background: msg.role === 'user' ? '#667eea' : '#64748b',
                          width: '24px',
                          height: '24px',
                          borderRadius: '50%',
                          display: 'flex',
                        ...
                        <div style={{ flex: 1 }}>
                          <div style={{ fontSize: '12px', color: '#64748b', marginBottom: '4px' }}>
                            {msg.role === 'user' ? 'Usuario' : 'Asistente'} • {formatDate(msg.timestamp)}
                          </div>
              ...
              <div style={{ marginBottom: '24px' }}>
                ...
                <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
                  {estadoOptions.slice(1).map(estado => (
                    <button
                      key={estado.value}
                      onClick={() => handleStatusChange(selectedTicket.id, estado.value)}
                      disabled={loading || selectedTicket.estado === estado.value}
                      style={{
                        background: selectedTicket.estado === estado.value ? getStatusColor(estado.value) : 'rgba(102, 126, 234, 0.1)',
                        color: selectedTicket.estado === estado.value ? 'white' : '#667eea',
                        border: 'none',
                        padding: '8px 16px',
                        borderRadius: '6px',
                        cursor: selectedTicket.estado === estado.value ? 'default' : 'pointer',
                        fontSize: '12px',
                        fontWeight: '500',
                        transition: 'all 0.3s ease',
                      ...
                      onMouseEnter={(e) => {
                        if (selectedTicket.estado !== estado.value && !loading) {
                          e.currentTarget.style.background = 'rgba(102, 126, 234, 0.2)';
                        }
                      }}
                      onMouseLeave={(e) => {
                    ...
                    <div>
                      ...
                      <div style={{ display: 'flex', gap: '8px' }}>
                        <button
                          ...
                          style={{
                            background: loading || !responseText.trim() ? '#94a3b8' : 'linear-gradient(135deg, #10b981 0%, #059669 100%)',
                            color: 'white',
                            border: 'none',
                            padding: '12px 24px',
                            borderRadius: '8px',
                            cursor: loading || !responseText.trim() ? 'not-allowed' : 'pointer',
                            fontSize: '14px',
                            fontWeight: '500',
                            display: 'flex',
                            alignItems: 'center',
